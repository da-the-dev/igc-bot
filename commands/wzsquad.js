...
module.exports =
    ...
    */
    async (args, msg, client) => {
        ...
        const create = args[1]
        if(!create) {
            const roomData = await lobbiesManager.roomFinder('warzone', 4, kd, msg)
            if(roomData)
                msg.channel.send(`<@&${constants.roles.wz}>`, roomData)
            else {
                const message = await embed.warning(msg, `${msg.author}, **SQUAD** комната по уровню не найдена, создаю...`)
                const roomData = await lobbiesManager.roomSpawner('warzone', 4, kd, msg)
